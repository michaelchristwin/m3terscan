...
import { getRecentBlocks, getWorldState } from "~/queries/meterscan.queries";
...
function Statistics() {
  ...
  const isLoading = worldState.isLoading || recentBlocks.isLoading;
  const isSuccess = worldState.isSuccess && recentBlocks.isSuccess;

  if (isLoading)
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 place-items-center gap-y-5">
        {Array.from({ length: 5 }).map((_, i) => (
          <Skeleton key={i} className="md:w-50.5 w-37.5 h-21.5 rounded-2xl" />
        ))}
      </div>
    );
